...
import com.freeme.memories.extra.bitmappool.SparseArrayBitmapPool.Node;

/**
 * Pool allowing the efficient reuse of bitmaps in order to avoid long
 * garbage collection pauses.
 */
public class AppBitmapPool {
    ...
    // SparseArrayBitmapPool instances to back the AppBitmapPool, which affords
    ...
    private static AppBitmapPool sInstance = new AppBitmapPool(CAPACITY_BYTES);
    private int                     mCapacityBytes;
    private SparseArrayBitmapPool[] mPools;
    private Pools.Pool<Node> mSharedNodePool = new Pools.SynchronizedPool<Node>(128);

    private AppBitmapPool(int capacityBytes) {
    ...
    public static AppBitmapPool getInstance() {
        return sInstance;
    }
